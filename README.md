# radio-cm-cutter

運用者向け: MP3音源からCM区間を検出し、CM除去済みMP3を作成するCLIツールです。

## Quickstart

### 1) 前提
- Python 3.11 推奨
- FFmpeg (`ffmpeg` / `ffprobe` をPATHに設定)
- `pip install -r requirements.txt`

### 2) ヘルプ確認
```bash
python radio_cm_cutter.py --help
```

### 3) 基本運用（単体）
```bash
python radio_cm_cutter.py detect input.mp3
python radio_cm_cutter.py cut input.mp3 --segments input_segments.csv
```

### 4) 基本運用（フォルダ）
```bash
python radio_cm_cutter.py process-folder "C:/radio" --recursive
```
- デフォルト出力先は `cut_output/YYYYMMDD_HHMMSS/` です（上書き事故防止）。
- 同一出力先の既存ファイルを上書きしたい場合のみ `--overwrite` を使用してください。

### 5) 学習/評価
```bash
python radio_cm_cutter.py train --labels data/labels/*.csv --out model/model.pkl
python radio_cm_cutter.py evaluate --labels data/labels/*.csv --model model/model.pkl
```

### 6) GUI起動（MVP / Windows想定）
```bash
python -m radio_cm_cutter_app
```
- 入力フォルダ、`model.pkl`、出力フォルダを選択して「実行」を押すだけで `process-folder` 相当を実行します。
- 未指定時の出力先は `cut_output/YYYYMMDD_HHMMSS/` です。
- 設定値（入力/出力/モデル/ffmpegパス）は `%APPDATA%/radio-cm-cutter/gui_settings.json` に保存され、次回起動時に復元されます。
- FFmpegが見つからない場合は、GUIに「自動で探した場所」と「次にやること」を表示し、`ffmpeg.exeを選択` / `導入手順を開く(README)` / `キャンセル`（+任意で`ffmpegをダウンロード`）を案内します。
- 任意の`ffmpeg.exe`指定は保存され、指定先が削除されていた場合は起動時/実行時に再案内します。


## Windows向けEXEビルド（PyInstaller）

### 目的
GUIアプリ（`python -m radio_cm_cutter_app`）を、Python未導入PCでも実行しやすい `EXE` として配布するための手順です。

### 事前準備
- Windows 10/11
- Python 3.11
- PowerShell 実行可能環境
- FFmpeg は**同梱しません**（PATH設定 or GUIで `ffmpeg.exe` 指定）

### ビルド手順（非エンジニア向け）
1. リポジトリを展開
2. `scripts\build_windows.bat` をダブルクリック
   - もしくは PowerShell で `./scripts/build_windows.ps1` を実行
3. 完了後、`dist/radio-cm-cutter-gui-windows.zip` が生成されます

ビルドスクリプトは次を自動実行します。
- 仮想環境作成（`.venv-build/`）
- 依存インストール（`requirements-app.txt`）
- PyInstaller実行（`scripts/radio_cm_cutter_gui.spec`）
- `dist/` へのEXE出力とzip化

### 配布物の中身
`radio-cm-cutter-gui-windows.zip`
- `radio-cm-cutter-gui.exe`
- `README.md`

> `dist/` と `build/` は生成物のためGit管理しません。

### インストーラ作成（Inno Setup / 任意）

PyInstallerで作成したGUI EXEを、Windows向けセットアップ形式（`.exe`インストーラ）にまとめる手順です。
※ Inno Setup本体はリポジトリに同梱しません。

#### 事前準備
- `scripts/build_windows.ps1` の前提と同様（Windows / Python 3.11 / PowerShell）
- Inno Setup 6（`ISCC.exe` が利用可能であること）

#### 作成手順
```powershell
./scripts/build_installer.ps1
```

このスクリプトは以下を自動実行します。
- `.venv-build/` を作成
- `requirements-app.txt` をインストール
- PyInstallerで `dist/radio-cm-cutter-gui.exe` を生成
- Inno Setupで `installer/RadioCmCutter.iss` をビルド
- `installer_out/RadioCmCutterSetup.exe` を出力

Inno Setupが未インストールの場合は、インストール手順を案内するエラーを表示して終了します。

#### 配布物の中身（インストーラ版）
`RadioCmCutterSetup.exe` でインストールされる内容:
- `radio-cm-cutter-gui.exe`
- `README.md`
- スタートメニューショートカット
- アンインストール情報（Windows「アプリと機能」から削除可能）

#### 更新時の手順（バージョン更新を含む）
1. `radio_cm_cutter_app/__init__.py` の `__version__` を更新
2. GUIの動作確認・必要な修正
3. `./scripts/build_installer.ps1` を実行してインストーラ再生成
4. 生成された `installer_out/RadioCmCutterSetup.exe` を配布

> `dist/` `build/` `installer_out/` は生成物のためGit管理しません。

### GitHub Actionsでのビルド
- workflow: `.github/workflows/build-windows-exe.yml`
- `windows-latest` 上で `scripts/build_windows.ps1` を実行し、
  `dist/radio-cm-cutter-gui-windows.zip` をartifactとして保存します。

## CLI一覧
- `detect`
- `detect-ml`
- `cut`
- `process-folder`
- `init-label-template`
- `train`
- `evaluate`

## 出力
`process-folder` の出力:
- `.../cut_output/<timestamp>/cut` : CM除去後MP3
- `.../cut_output/<timestamp>/segments` : 検出CSV
- `.../cut_output/<timestamp>/reports` : HTMLレポート

## トラブルシュート

### ffmpeg/ffprobe が見つからない
- 症状: 実行時に `Required command not found`。
- 対応: FFmpegをインストールし、`ffmpeg` と `ffprobe` のPATHを通してください。
- GUIでは検出時に以下を実施します。
  - 自動検索結果（PATH、前回指定パス、`%LOCALAPPDATA%/radio-cm-cutter/ffmpeg/`）の表示
  - 次の行動を明示（`ffmpeg.exeを選択`、`導入手順を開く(README)`）
  - 任意でFFmpegをダウンロードして展開（保存先: `%LOCALAPPDATA%/radio-cm-cutter/ffmpeg/`）
  - ダウンロード失敗時は理由（ネットワークエラーなど）を表示


### EXEがウイルス対策ソフトで誤検知される
- 症状: 実行ファイルが隔離/ブロックされる。
- 対応: PyInstaller製EXEで誤検知される場合があります。社内配布時は署名・配布経路のホワイトリスト化を検討してください。

### EXEでffmpegが見つからない
- 症状: `Required command not found` またはGUIでffmpeg関連エラー。
- 対応: FFmpeg本体は同梱していないため、PATH設定またはGUIで `ffmpeg.exe` を指定してください。
- 補足: GUIで指定したパスは `%APPDATA%/radio-cm-cutter/gui_settings.json` に保存され、次回起動時に復元されます。
- GUIには「診断」ボタンがあり、Python/アプリ版/OS/CPU、ffmpeg検出、scikit-learn版、model.pkl情報、設定/ログ保存先を確認できます。
- GUIの「診断レポートzipを出力」から、`logs/` + 設定 + `versions.txt` をまとめたZIPを作成できます。
- GUIには「ログを開く」ボタンがあり、ログフォルダを直接開けます。
- ログは `%LOCALAPPDATA%/radio-cm-cutter/logs/`（フォールバック: `%APPDATA%`）に保存されます。`startup.log`（起動系）と `run.log`（実行系）を分離しています。

### モデルが読み込めない
- 症状: `Model not found` / モデルロード例外。
- 対応: `--model` のパス、`config.json` の `model_path`、モデルファイルの破損有無を確認してください。

### 入力ファイル/フォルダが見つからない
- 症状: `FileNotFoundError` / `Folder not found`。
- 対応: 絶対パスで指定し、存在とアクセス権限を確認してください。

### 出力で失敗する
- 症状: 権限エラーや書き込み失敗。
- 対応: 書き込み可能な場所を指定するか、別の `--out-dir` / `--out` を利用してください。

## 運用ポリシー
- `model/` と `cut_output/` はGit管理しない運用を推奨（`.gitignore`で除外）。
- 検出ロジック・特徴量・しきい値の意味は変更せず、構造改善中心で保守してください。

## GUI 手動テスト（最小）
1. `python -m radio_cm_cutter_app` を起動。
2. 入力フォルダに MP3 を1つ以上含むディレクトリを選択（例: `C:/radio_input`）。
3. モデルは既定の `model/model.pkl` を使うか、任意の `.pkl` を指定。
4. 出力フォルダは空欄なら `cut_output/<timestamp>/`、または任意ディレクトリを指定。
5. 実行モードを選択（「検出だけ」または「検出してカット」）。
6. 「実行」を押し、ログ欄で進行を確認（「検出してカット」では検出サマリ確認ダイアログでOKするとカット実行）。
7. 完了後、`<出力>/reports/*_report.html` と `<出力>/ml_report.html` が生成されることを確認。
8. レポートは自動で開かれないことを確認し、必要なときだけGUIの「ml_report.html を開く」ボタンで開けることを確認。

## 配布前チェックリスト
- [ ] WindowsのPython未導入PC（またはクリーンVM）で `radio-cm-cutter-gui.exe` が起動する。
- [ ] `scripts/smoke_test_windows.ps1` を実行し、`dist` のEXEに対する基本スモークテストを通す。
- [ ] FFmpeg未導入状態で実行し、エラーが「ffmpeg/ffprobe が見つからない」旨で分かりやすく表示される。
- [ ] FFmpeg導入後、GUIのFFmpeg指定（任意）とPATH検出の双方で処理できる。
- [ ] GUIの「診断」→「診断レポートzipを出力」でZIPを取得し、`logs/`, `config/`, `versions.txt` が含まれることを確認。
